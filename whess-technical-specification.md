...
| Refresh mất phiên | Reload tab không phục hồi room/ván, app quay về Lobby. |
...
| Session persistence | Không cần vì refresh được xem là mất phiên. |
...
| ADR-07 | Không persist session ở browser hoặc database. | Web demo chỉ chơi một lần; refresh là mất phiên và quay về Lobby. |
...
| Room Manager | Quản lý phòng, người chơi, leave/disconnect và cleanup RAM. |
...
| Result Overlay | Xem kết quả và phân tích | Kết quả, loading AI, ELO, ECO, stats, CPL chart, blunder accordion, markdown explanation, về Lobby/tạo phòng mới. |
...
| Session | Chỉ giữ `roomId`, `color` trong memory state; không dùng storage để phục hồi sau refresh. |
...
| Module | Trách nhiệm |
|:--|:--|
| Socket Gateway | Định nghĩa Socket.IO events, map socket hiện tại với player slot, emit room state. |
| Room Manager | Tạo phòng, join, leave, disconnect, cleanup, lưu state RAM. |
...
| Người chơi | `players.white`, `players.black` | Mỗi slot có `socketId`, `color`, `connected`; không có token phục hồi. |
...
| Disconnect/leave khi đang chơi | Người rời ván thua ngay với `reason=abandon`. |
...
| Event | Payload | Ghi chú |
|:--|:--|:--|
| `room_created` | `{ roomId, color }` | Creator là `white`. |
| `room_joined` | `{ roomId, color, state }` | Joiner là `black`. |
...
stateDiagram-v2
    ...
    Waiting --> [*]: leave/disconnect/cleanup
    Playing --> Playing: valid_move
    Playing --> Finished: game_over
    Playing --> Finished: leave/disconnect/abandon
    Finished --> [*]: cleanup
```
...
| `warning` | `#F2B84B` | Chờ/timeout sắp hết. |
...
| AI error | Hiển thị cảnh báo AI không khả dụng, vẫn cho về Lobby/tạo phòng mới. |

---

## 13. Refresh, Disconnect & Cleanup

Không có database, không có token phục hồi, không có business reconnect. Room state chỉ nằm trong RAM của Web Game Server.

| Tình huống | Hành vi |
|:--|:--|
| Reload tab ở bất kỳ màn nào | App mới mở ở Lobby, không gửi event phục hồi. |
| Disconnect/leave khi `waiting` | Xóa room ngay hoặc đánh dấu inactive để cleanup. |
| Disconnect/leave khi `playing` | Dừng clock, set `status=finished`, đối thủ thắng, emit `game_over` với `reason=abandon`, sau đó gọi AI nếu đủ dữ liệu. |
| Disconnect/leave khi `finished` | Không đổi kết quả; room chờ cleanup. |
| Cả hai rời phòng | Room được cleanup khỏi RAM. |
...
Room đã kết thúc là terminal. Không có `play_again`, không có `game_reset`; muốn chơi ván mới thì tạo room mới từ Lobby.
...
| Disconnect/leave | `roomId`, `color`, `status`, `outcome`. |
| Cleanup | `roomId`, `reason`. |
...
| AC-16 | Refresh tab không phục hồi ván; app quay về Lobby. |
| AC-17 | Disconnect/refresh khi đang chơi kết thúc ván bằng `reason=abandon` cho đối thủ. |
| AC-18 | Room inactive hoặc finished được cleanup khỏi RAM. |
...
| Refresh/disconnect | Reload ở Lobby/Waiting/Playing, opponent refresh khi đang chơi, cleanup room terminal. |
